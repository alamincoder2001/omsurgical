						...
						<div class="col-sm-5">
							<form v-on:submit.prevent="addToCart">
								<div class="form-group">
									...
									<div class="col-sm-8">
										<v-select v-bind:options="onlyBranch" v-model="selectedWarehouse" label="Brunch_name" v-on:input="onChangeWarehouse"></v-select>
									</div>
								...
								<div class="form-group">
									...
									<div class="col-sm-9">
										<v-select v-bind:options="models" v-model="selectedModel" label="model_name" v-on:input="modelOnChange"></v-select>
									</div>
								</div>
				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					<thead>
						<tr class="">
							...
							<th style="width:10%;color:#000;">P. Code</th>
							<th style="width:20%;color:#000;">Product Name</th>
							<th style="width:10%;color:#000;">Model Name</th>
					...
					<tbody style="display:none;" v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
						<tr v-for="(product, sl) in cart">
							...
							<td>
								<input type="text" v-model="product.quantity" class="form-control" v-on:input.prevent="updateCart(product)" style="color: #000;text-align: center;">
							</td>
							<!-- <td>{{ product.salesRate }}</td> -->
							<td>
								<input type="text" v-model="product.salesRate" class="form-control" v-on:input.prevent="updateCart(product)" style="color: #000;text-align: center;">
							</td>
							<td>{{ product.total }}</td>
		...
		methods: {
			modelOnChange() {
				...
				if(this.selectedModel.model_id != ''){
					...
					isService: this.sales.isService, model_id: this.selectedModel.model_id
					}).then(res => {
			...
			addToCart() {
				let product = {
					...
					model: this.selectedProduct.model_name,
				...
				await axios.post('/get_sales', {
					...
					r.saleDetails.forEach(product => {
						let cartProduct = {
							...
							branchId: product.SaleDetails_BranchId,
						}

...
<?php
    ...
    {
        $models = $this->db->query("select * from tbl_model")->result();
    ...
    public function modelPage()
    {
        ...
        $data['model'] =  $this->Billing_model->select_model($this->brunch);
        $data['content'] = $this->load->view('Administrator/add_model', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function addModel()
    {
        ...
        $query = $this->db->query("select * from tbl_model where model_name = '$data->model_name' ");

        if ($query->num_rows() > 0) {
            $res = ['status' => false, 'message' => 'Model name already exist'];
            echo json_encode($res);
            exit;
        } else {
            $val = array(
            ...
            $this->db->insert('tbl_model', $val);
            $res = ['status' => true, 'message' => 'Model added successfully!'];
            echo json_encode($res);
        }
    }
    ...
    public function Update_Model()
    {
        ...
        $query = $this->db->query("select * from tbl_model where model_name = '$data->model_name' and model_id != $data->model_id");
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->db->where('model_id', $data->model_id)->update('tbl_model', $val);
            $res = ['status' => true, 'message' => 'Model Update successfully!'];
            echo json_encode($res);
        }
    }
    ...
    {
        $result = $this->db->query("delete from tbl_model where model_id = '$id'");

        if ($result) {
            $res = ['status' => true, 'message' => 'Model Delete successfully!'];
            echo json_encode($res);
        } else {
            $res = ['status' => false, 'message' => 'Sorry! Something going wrong! try again later'];
            echo json_encode($res);
        }
    }
}

    ...
    public function getPurchases()
    {
        ...
        if (isset($data->purchaseId) && $data->purchaseId != null) {
            ...
            $res['purchaseDetails'] = $this->db->query("
                ...
                join tbl_model m on m.model_id = p.model
        ...
        try {
            ...
            foreach ($data->cartProducts as $product) {
                $purchaseDetails = array(
                    ...
                    'UpdateBy' => $this->session->userdata("FullName"),
                    'UpdateTime' => date('Y-m-d H:i:s'),
                    'PurchaseDetails_branchID' => $this->brunch
                );

                $this->db->insert('tbl_purchasedetails', $purchaseDetails);

                $previousStock = $this->mt->productStock($product->productId, $this->brunch);

                $inventoryCount = $this->db->query("select * from tbl_currentinventory where product_id = ? and branch_id = ?", [$product->productId, $this->brunch])->num_rows();
                if ($inventoryCount == 0) {
                    $inventory = array(
                        'product_id' => $product->productId,
                        'purchase_quantity' => $product->quantity,
                    ...
                    $this->db->query("
                        ...
                        where product_id = ?
                        and branch_id = ?
                    ", [$product->quantity, $product->productId, $this->brunch]);
                }
